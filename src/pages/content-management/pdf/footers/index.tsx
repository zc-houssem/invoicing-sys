...
export default function Page() {
  ...
  React.useEffect(() => {
    setIntro?.(
      tContentManagement('pdf.footers.title', { defaultValue: 'Footers' }),
      tContentManagement('pdf.footers.coming_soon', { defaultValue: 'Footer management coming soon.' })
    );
    setRoutes?.([
  ...
  return (
    <PdfSettingsLayout>
      <div className="p-4 bg-white dark:bg-zinc-900 rounded-lg shadow-sm border border-border">
        <h1 className="text-2xl font-bold mb-4">{tContentManagement('pdf.footers.title', { defaultValue: 'Footers' })}</h1>
        <p className="text-muted-foreground">{tContentManagement('pdf.footers.coming_soon', { defaultValue: 'Footer management coming soon.' })}</p>
      </div>
    </PdfSettingsLayout>
  );
}
