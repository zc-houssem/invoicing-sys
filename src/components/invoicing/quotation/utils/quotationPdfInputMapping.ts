...
export const buildQuotationPdfInputMapping = (
  q: any,
  ...
  return {
    ...
    'quotation.number': q?.sequence || '',
    'quotation.object': q?.object || '',
    'quotation.issueDate': q?.date ? new Date(q.date).toLocaleDateString() : '',
    'quotation.dueDate': q?.dueDate ? new Date(q.dueDate).toLocaleDateString() : '',
    'quotation.status': q?.status || '',
    'quotation.generalConditions': q?.generalConditions || '',
    'quotation.notes': q?.notes || '',
    ...
    // Totals
    'quotation.subtotal': q?.totalExcludingTaxes?.toString() || '',
    'quotation.grandTotal': q?.totalIncludingTaxes?.toString() || ''
  };
};
