    ...
    <Command
          ...
          {!hideClearAllButton &&
            ...
            selected.filter((s) => s.fixed).length !== selected.length && (
              <button
                ...
                className={cn(
                  'absolute right-1.5 top-1/2 -translate-y-1/2 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-none disabled:pointer-events-none',
                  disabled && 'hidden'
                )}
                disabled={disabled}
      ...
      <div className="relative">
        {open && (
          <div
            className={cn(
              'absolute top-2 z-50 w-full overflow-hidden rounded-md border bg-popover p-1 text-popover-foreground shadow-md outline-none animate-in fade-in-0 zoom-in-95'
            )}>
            <CommandList
                      ...
                      {dropdowns.map((option) => (
                        <CommandItem
                          ...
                          className={cn(
                            ...
                            option.disable ? 'pointer-events-none opacity-50' : 'cursor-pointer'
                          )}>
                          {option.label}
                        </CommandItem>
                      ))}
